import { ArrowRight, UserPlus, Code } from 'lucide-react';
import { useAgent, AgentType } from '../context/AgentContext';
...
export function Main() {
  ...
  // Local state to control the main content view
  const [mainView, setMainView] = useState<AgentType | 'welcome' | 'embedded'>('welcome');

  // Effect to sync mainView when activeAgent changes (from sidebar)
  useEffect(() => {
    if (activeAgent === 'sales' || activeAgent === 'onboarding') {
      setMainView(activeAgent);
    } else {
      // Default to welcome if 'llmaserviceinfo' or null is selected
      setMainView('welcome');
    }
  }, [activeAgent]);
  ...
  // Determine what component to render in the main area
      ...
      default:
        ...
        return (
          <div className="flex flex-col h-full">
            <main className="flex-1 container mx-auto px-4 py-8 md:px-6 overflow-y-auto">
              <div className="content-area mb-10">
                ...
                <div className="grid md:grid-cols-3 gap-6 mt-8">
                  <div className="agent-card p-6">
                    ...
                    <button 
                      onClick={() => {
                        setActiveAgent('sales'); // Update context for sidebar
                        setMainView('sales');   // Update local state for main view
                      }}
                  ...
                  <div className="agent-card p-6">
                    ...
                    <button 
                      onClick={() => {
                        setActiveAgent('onboarding'); // Update context for sidebar
                        setMainView('onboarding');   // Update local state for main view
                      }}
                  ...
                  <div className="agent-card p-6">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-xl font-semibold">Simple IFrame Embedding</h3>
                    </div>
                    <p className="text-gray-600 mb-4">
                      Easily embed our Agent Panel in your own applications using a simple iframe. Configure button formats, regex patterns, and markdown to customize the embedded experience.
                    </p>
                    <button 
                      onClick={() => setMainView('embedded')} // ONLY update local state
